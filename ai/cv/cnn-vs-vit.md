...
아주 날카로운 질문입니다! ViT가 등장했을 때 다들 "이제 CNN은 끝났다"라고 생각했지만, 몇 년이 지난 지금도 CNN은 여전히 현역이며 건재합니다.

그 이유는 \*\*"가성비"\*\*와 "환경" 때문입니다.

쉽게 비유하자면 \*\*CNN은 "노련한 기술자"\*\*이고, \*\*ViT는 "천재형 연구원"\*\*입니다. 상황에 따라 누구를 써야 할지가 명확히 갈립니다. 왜 아직도 CNN을 쓰는지 4가지 핵심 이유로 정리해 드릴게요.

***

#### 1. "데이터 헝그리" (Data Efficiency)

ViT의 가장 큰 약점입니다. ViT는 데이터가 엄청나게 많아야만 잘합니다.

* ViT (천재형): "나한테 사전 지식(Inductive Bias) 따윈 주지 마. 대신 교과서 \*\*3억 권(JFT-300M)\*\*을 주면 내가 스스로 우주의 진리를 깨우칠게."
  * $$ $\rightarrow$ $$ 데이터가 적으면 과적합(Overfitting)되거나 학습이 아예 안 됩니다.
* CNN (노련한 기술자): "난 '이미지란 원래 이런 거야'라는 기본 상식(Inductive Bias)을 갖고 있어. 교과서 \*\*1,000권(ImageNet-1k)\*\*만 줘도 충분히 잘해."
  * $$ $\rightarrow$ $$ 현실 세계 대부분의 프로젝트는 데이터가 부족합니다. (희귀병 엑스레이, 공장 불량품 사진 등). 이럴 땐 CNN이 압도적으로 유리합니다.

> 위 그래프를 보면 데이터 양(x축)이 적을 땐 CNN(ResNet)이 이기다가, 데이터가 아주 많아지면 ViT가 역전하는 것을 볼 수 있습니다.

#### 2. 고해상도 이미지 처리 (Resolution)

이미지가 커질수록 ViT는 연산량이 기하급수적으로 폭발합니다.
